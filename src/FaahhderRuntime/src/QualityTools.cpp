#include "Faahhder/QualityTools.hpp"
...
std::string ToString(QualityItemKind kind) {
    switch (kind) {
    case QualityItemKind::Frame: return "Frame";
    case QualityItemKind::Memory: return "Memory";
    case QualityItemKind::AssetCheck: return "AssetCheck";
    case QualityItemKind::SceneCheck: return "SceneCheck";
    case QualityItemKind::ScriptCheck: return "ScriptCheck";
    case QualityItemKind::ExportCheck: return "ExportCheck";
    case QualityItemKind::Warning: return "Warning";
    case QualityItemKind::Error: return "Error";
    case QualityItemKind::Profile: return "Profile";
    case QualityItemKind::Summary: return "Summary";
    }
    return "Frame";
}

bool TryParseQualityItemKind(const std::string& text, QualityItemKind& out) {
    if (text == "Frame") { out = QualityItemKind::Frame; return true; }
    if (text == "Memory") { out = QualityItemKind::Memory; return true; }
    if (text == "AssetCheck") { out = QualityItemKind::AssetCheck; return true; }
    if (text == "SceneCheck") { out = QualityItemKind::SceneCheck; return true; }
    if (text == "ScriptCheck") { out = QualityItemKind::ScriptCheck; return true; }
    if (text == "ExportCheck") { out = QualityItemKind::ExportCheck; return true; }
    if (text == "Warning") { out = QualityItemKind::Warning; return true; }
    if (text == "Error") { out = QualityItemKind::Error; return true; }
    if (text == "Profile") { out = QualityItemKind::Profile; return true; }
    if (text == "Summary") { out = QualityItemKind::Summary; return true; }
    return false;
}

int QualityPlan::Add(QualityItemKind kind, std::string name, std::filesystem::path path, std::string tag) {
    QualityItem value;
    value.id = nextId_++;
    value.kind = kind;
...
bool QualityPlan::Upsert(int id, QualityItem value) {
    value.id = id;
...
bool QualityPlan::Patch(int id, const QualityItemPatch& patch) {
    auto* entry = Find(id);
...
bool QualityPlan::Remove(int id) {
...
bool QualityPlan::Rename(int id, std::string name) {
    auto* entry = Find(id);
    if (!entry) return false;
...
bool QualityPlan::Retag(int id, std::string tag) {
    auto* entry = Find(id);
    if (!entry) return false;
...
bool QualityPlan::Enable(int id, bool enabled) {
    auto* entry = Find(id);
    if (!entry) return false;
...
bool QualityPlan::MoveBefore(int id, int beforeId) {
...
bool QualityPlan::MoveAfter(int id, int afterId) {
    auto it = std::find_if(entries_.begin(), entries_.end(), [id](const auto& entry) { return entry.id == id; });
...
QualityItem* QualityPlan::Find(int id) {
    for (auto& entry : entries_) if (entry.id == id) return &entry;
    return nullptr;
}

const QualityItem* QualityPlan::Find(int id) const {
    for (const auto& entry : entries_) if (entry.id == id) return &entry;
    return nullptr;
}

const QualityItem* QualityPlan::FindByName(const std::string& name) const {
    for (const auto& entry : entries_) if (entry.name == name) return &entry;
    return nullptr;
}

std::vector<QualityItem> QualityPlan::FindByKind(QualityItemKind kind) const {
    std::vector<QualityItem> out;
    for (const auto& entry : entries_) if (entry.kind == kind) out.push_back(entry);
    return out;
}

std::vector<QualityItem> QualityPlan::FindByTag(const std::string& tag) const {
    std::vector<QualityItem> out;
    for (const auto& entry : entries_) if (entry.tag == tag) out.push_back(entry);
    return out;
}

std::vector<QualityItem> QualityPlan::Search(const std::string& text) const {
    std::vector<QualityItem> out;
...
std::vector<QualityItem> QualityPlan::Enabled() const {
    std::vector<QualityItem> out;
    for (const auto& entry : entries_) if (entry.enabled) out.push_back(entry);
    return out;
}

std::vector<QualityItem> QualityPlan::Sorted() const {
    auto out = entries_;
...
std::vector<QualityItem> QualityPlan::Entries() const { return entries_; }
std::size_t QualityPlan::Size() const { return entries_.size(); }
bool QualityPlan::Empty() const { return entries_.empty(); }

int QualityPlan::TotalWeight() const {
    int total = 0;
    for (const auto& entry : entries_) if (entry.enabled) total += entry.weight;
    return total;
}

int QualityPlan::NextId() const { return nextId_; }
void QualityPlan::Clear() { entries_.clear(); nextId_ = 1; }

std::string QualityPlan::Serialize() const {
...
bool QualityPlan::Deserialize(const std::string& text) {
    entries_.clear();
    nextId_ = 1;
    ...
    while (std::getline(input, line)) {
        ...
        if (!TryParseQualityItemKind(parts[1], entry.kind)) return false;
        entry.name = parts[2];
        entry.path = parts[3];
        entry.tag = parts[4];
...
std::string QualityPlan::Report() const {
